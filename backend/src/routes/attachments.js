...
router.post(
  '/bulk-import',
  wrapAsync(async (req, res) => {
    await AttachmentsService.bulkImport(req, res, true, req.headers.referer);
    const payload = true;
    res.status(200).send(payload);
  }),
);
...
router.put(
  '/:id',
  wrapAsync(async (req, res) => {
    await AttachmentsService.update(
      req.body.data,
      req.body.id,
      req.currentUser,
    );
...
router.get(
  '/:id',
  wrapAsync(async (req, res) => {
    const payload = await AttachmentsDBApi.findBy({ id: req.params.id });
