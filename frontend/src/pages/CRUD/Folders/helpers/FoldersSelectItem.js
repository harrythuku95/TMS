...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/folders/autocomplete`, { params });
  return response.data;
}

const FoldersSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.folders.hasPermissionToCreate,
});

export default connect(select)(FoldersSelectItem);
