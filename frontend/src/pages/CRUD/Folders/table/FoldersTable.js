...
import * as dataFormat from 'pages/CRUD/Folders/table/FoldersDataFormatters';

import actions from 'actions/folders/foldersListActions';
import actionsFolders from 'actions/folders/foldersFormActions';
...
const FoldersTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'folder_id', title: 'folder_id' },
  ]);
  ...
  const count = useSelector((store) => store.folders.list.count);
  const modalOpen = useSelector((store) => store.folders.list.modalOpen);
  const rows = useSelector((store) => store.folders.list.rows);
  const idToDelete = useSelector((store) => store.folders.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsFolders.doUpdate(id, data));
  };
  ...
  const sendFoldersCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('folders/bulk-import', data, {
        headers: {
  ...
  const getFoldersCSV = async () => {
    const response = await axios({
      url: '/folders?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
  ...
  return (
    <div>
      <Widget title={humanize('Folders')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/folders/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Folders'
                  : 'http://localhost:8080/api-docs/#/Folders'
              }
            >
