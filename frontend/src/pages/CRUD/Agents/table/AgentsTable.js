...
import * as dataFormat from 'pages/CRUD/Agents/table/AgentsDataFormatters';

import actions from 'actions/agents/agentsListActions';
import actionsAgents from 'actions/agents/agentsFormActions';
...
const AgentsTable = () => {
  ...
  const count = useSelector((store) => store.agents.list.count);
  const modalOpen = useSelector((store) => store.agents.list.modalOpen);
  const rows = useSelector((store) => store.agents.list.rows);
  const idToDelete = useSelector((store) => store.agents.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsAgents.doUpdate(id, data));
  };
  ...
  return (
    <div>
      <Widget title={humanize('Agents')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/agents/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Agents'
                  : 'http://localhost:8080/api-docs/#/Agents'
              }
            >
