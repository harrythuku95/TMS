...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/agents/autocomplete`, { params });
  return response.data;
}

const AgentsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.agents.hasPermissionToCreate,
});

export default connect(select)(AgentsSelectItem);
