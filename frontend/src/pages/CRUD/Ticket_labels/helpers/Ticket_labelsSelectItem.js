...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/ticket_labels/autocomplete`, { params });
  return response.data;
}

const Ticket_labelsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.ticket_labels.hasPermissionToCreate,
});

export default connect(select)(Ticket_labelsSelectItem);
