...
import * as dataFormat from 'pages/CRUD/Ticket_labels/table/Ticket_labelsDataFormatters';

import actions from 'actions/ticket_labels/ticket_labelsListActions';
import actionsTicket_labels from 'actions/ticket_labels/ticket_labelsFormActions';
...
const Ticket_labelsTable = () => {
  ...
  const count = useSelector((store) => store.ticket_labels.list.count);
  const modalOpen = useSelector((store) => store.ticket_labels.list.modalOpen);
  const rows = useSelector((store) => store.ticket_labels.list.rows);
  const idToDelete = useSelector(
    (store) => store.ticket_labels.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsTicket_labels.doUpdate(id, data));
  };
  ...
  const sendTicket_labelsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('ticket_labels/bulk-import', data, {
        headers: {
  ...
  const getTicket_labelsCSV = async () => {
    const response = await axios({
      url: '/ticket_labels?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'ticket_labelsCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Ticket_labels')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/ticket_labels/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Ticket_labels'
                  : 'http://localhost:8080/api-docs/#/Ticket_labels'
              }
            >
              API documentation for ticket_labels
            </LinkMaterial>
          </div>
        )}
      </Widget>
...
export default Ticket_labelsTable;
