...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/webhooks/autocomplete`, { params });
  return response.data;
}

const WebhooksSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.webhooks.hasPermissionToCreate,
});

export default connect(select)(WebhooksSelectItem);
