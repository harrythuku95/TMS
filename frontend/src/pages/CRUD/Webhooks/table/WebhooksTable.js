...
import * as dataFormat from 'pages/CRUD/Webhooks/table/WebhooksDataFormatters';

import actions from 'actions/webhooks/webhooksListActions';
import actionsWebhooks from 'actions/webhooks/webhooksFormActions';
...
const WebhooksTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'webhook_id', title: 'webhook_id' },
  ]);
  ...
  const count = useSelector((store) => store.webhooks.list.count);
  const modalOpen = useSelector((store) => store.webhooks.list.modalOpen);
  const rows = useSelector((store) => store.webhooks.list.rows);
  const idToDelete = useSelector((store) => store.webhooks.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsWebhooks.doUpdate(id, data));
  };
  ...
  const sendWebhooksCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('webhooks/bulk-import', data, {
        headers: {
  ...
  const getWebhooksCSV = async () => {
    const response = await axios({
      url: '/webhooks?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
  ...
  return (
    <div>
      <Widget title={humanize('Webhooks')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/webhooks/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Webhooks'
                  : 'http://localhost:8080/api-docs/#/Webhooks'
              }
            >
