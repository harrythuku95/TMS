...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/ticket_counts/autocomplete`, { params });
  return response.data;
}

const Ticket_countsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.ticket_counts.hasPermissionToCreate,
});

export default connect(select)(Ticket_countsSelectItem);
