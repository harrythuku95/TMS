...
import * as dataFormat from 'pages/CRUD/Ticket_counts/table/Ticket_countsDataFormatters';

import actions from 'actions/ticket_counts/ticket_countsListActions';
import actionsTicket_counts from 'actions/ticket_counts/ticket_countsFormActions';
...
const Ticket_countsTable = () => {
  ...
  const count = useSelector((store) => store.ticket_counts.list.count);
  const modalOpen = useSelector((store) => store.ticket_counts.list.modalOpen);
  const rows = useSelector((store) => store.ticket_counts.list.rows);
  const idToDelete = useSelector(
    (store) => store.ticket_counts.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsTicket_counts.doUpdate(id, data));
  };
  ...
  const sendTicket_countsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('ticket_counts/bulk-import', data, {
        headers: {
  ...
  const getTicket_countsCSV = async () => {
    const response = await axios({
      url: '/ticket_counts?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'ticket_countsCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Ticket_counts')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/ticket_counts/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Ticket_counts'
                  : 'http://localhost:8080/api-docs/#/Ticket_counts'
              }
            >
              API documentation for ticket_counts
            </LinkMaterial>
          </div>
        )}
      </Widget>
...
export default Ticket_countsTable;
