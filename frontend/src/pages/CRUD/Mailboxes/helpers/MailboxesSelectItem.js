...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/mailboxes/autocomplete`, { params });
  return response.data;
}

const MailboxesSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.mailboxes.hasPermissionToCreate,
});

export default connect(select)(MailboxesSelectItem);
