...
import * as dataFormat from 'pages/CRUD/Mailboxes/table/MailboxesDataFormatters';

import actions from 'actions/mailboxes/mailboxesListActions';
import actionsMailboxes from 'actions/mailboxes/mailboxesFormActions';
...
const MailboxesTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'mailbox_id', title: 'mailbox_id' },
  ]);
  ...
  const count = useSelector((store) => store.mailboxes.list.count);
  const modalOpen = useSelector((store) => store.mailboxes.list.modalOpen);
  const rows = useSelector((store) => store.mailboxes.list.rows);
  const idToDelete = useSelector((store) => store.mailboxes.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsMailboxes.doUpdate(id, data));
  };
  ...
  const sendMailboxesCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('mailboxes/bulk-import', data, {
        headers: {
  ...
  const getMailboxesCSV = async () => {
    const response = await axios({
      url: '/mailboxes?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'mailboxesCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Mailboxes')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/mailboxes/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Mailboxes'
                  : 'http://localhost:8080/api-docs/#/Mailboxes'
              }
            >
              API documentation for mailboxes
            </LinkMaterial>
          </div>
        )}
      </Widget>
