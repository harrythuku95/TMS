...
import * as dataFormat from 'pages/CRUD/Tickets/table/TicketsDataFormatters';

import actions from 'actions/tickets/ticketsListActions';
import actionsTickets from 'actions/tickets/ticketsFormActions';
...
const TicketsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'ticket_id', title: 'ticket_id' },
  ]);
  ...
  const count = useSelector((store) => store.tickets.list.count);
  const modalOpen = useSelector((store) => store.tickets.list.modalOpen);
  const rows = useSelector((store) => store.tickets.list.rows);
  const idToDelete = useSelector((store) => store.tickets.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsTickets.doUpdate(id, data));
  };
  ...
  return (
    <div>
      <Widget title={humanize('Tickets')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/tickets/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Tickets'
                  : 'http://localhost:8080/api-docs/#/Tickets'
              }
            >
              API documentation for tickets
            </LinkMaterial>
          </div>
        )}
      </Widget>
