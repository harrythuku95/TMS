...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/attachments/autocomplete`, { params });
  return response.data;
}

const AttachmentsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.attachments.hasPermissionToCreate,
});

export default connect(select)(AttachmentsSelectItem);
