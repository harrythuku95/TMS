...
import * as dataFormat from 'pages/CRUD/Attachments/table/AttachmentsDataFormatters';

import actions from 'actions/attachments/attachmentsListActions';
import actionsAttachments from 'actions/attachments/attachmentsFormActions';
...
const AttachmentsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'attachment_id', title: 'attachment_id' },
  ]);
  ...
  const count = useSelector((store) => store.attachments.list.count);
  const modalOpen = useSelector((store) => store.attachments.list.modalOpen);
  const rows = useSelector((store) => store.attachments.list.rows);
  const idToDelete = useSelector((store) => store.attachments.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsAttachments.doUpdate(id, data));
  };
  ...
  const sendAttachmentsCSV = async () => {
  ...
  return (
    <div>
      <Widget title={humanize('Attachments')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/attachments/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Attachments'
                  : 'http://localhost:8080/api-docs/#/Attachments'
              }
            >
              API documentation for attachments
            </LinkMaterial>
          </div>
        )}
      </Widget>
