  ...
  doCreate: (values) => async (dispatch) => {
    try {
      ...
      axios.post('/customers', { data: values }).then((res) => {
        ...
        showSnackbar({ type: 'success', message: 'Customers created' });
        dispatch(push('/admin/customers'));
      });
    } catch (error) {
      Errors.handle(error);
  ...
  doUpdate: (id, values, isProfile) => async (dispatch, getState) => {
    try {
      ...
      if (isProfile) {
        ...
        showSnackbar({ type: 'success', message: 'Customers updated' });
        dispatch(push('/admin/customers'));
      }
    } catch (error) {
      Errors.handle(error);
      showSnackbar({ type: 'error', message: 'Customers update error' });
      dispatch({
